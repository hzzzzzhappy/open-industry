...
from model.pointmae.M3DM.cpu_knn import KNN
import os
import numpy as np


def _load_ckpt(path, map_location=None):
    """torch.load for legacy checkpoints (PyTorch >=2.6 defaults weights_only=True)."""
    try:
...
class Model1(torch.nn.Module):
    ...
    def __init__(self, device, rgb_backbone_name='vit_base_patch8_224_dino', out_indices=None, checkpoint_path='',
        ...
        up4_dir = os.path.dirname(os.path.dirname(current_dir))
        # up4_dir = up4_dir + "/pointmae/"
        
        if xyz_backbone_name=='Point_MAE':
            self.xyz_backbone=PointTransformer(group_size=group_size, num_group=num_group)
            self.xyz_backbone.load_model_from_ckpt(up4_dir+"/feature_extract_checkpoint/pointmae_pretrain.pth")
        elif xyz_backbone_name=='Point_BERT':
            self.xyz_backbone=PointTransformer(group_size=group_size, num_group=num_group, encoder_dims=256)
            self.xyz_backbone.load_model_from_pb_ckpt(up4_dir+"/feature_extract_checkpoint/Point-BERT.pth")
...
def fps(data, number):
    """Furthest-point sampling. data: [B, N, 3] CUDA float32; number <= N."""
    assert data.ndim == 3, f"fps() expect 3D tensor, got {data.ndim}D"
    B, N, C = data.shape
    assert C == 3, f"fps() last dim must be 3, got {C}"
    assert data.is_cuda, "fps() expect data on CUDA"
    assert data.dtype == torch.float32, f"fps() expect float32, got {data.dtype}"

    if N < number:
        print(f"[WARN][fps] requested number={number} > N={N}; using number=N")
        number = N

    if not torch.isfinite(data).all():
        raise RuntimeError("[fps] data contains NaN or Inf (invalid for CUDA FPS)")

    data_c = data.contiguous()
    try:
        fps_idx = pointnet2_utils.furthest_point_sample(data_c, number)
    except RuntimeError as e:
        # CUDA kernel failed (illegal memory access). Fall back to a CPU implementation.
        print("[WARN] pointnet2_ops.furthest_point_sample failed, falling back to CPU implementation:", e)
        # Move to CPU and use numpy-based FPS (may be slow for large N)
        data_cpu = data_c.to('cpu').numpy()
        B, N, _ = data_cpu.shape
        fps_idx_list = []
        for b in range(B):
            pts = data_cpu[b]
            # greedy FPS
            selected = [0]
            dist = ((pts - pts[0]) ** 2).sum(axis=1)
            for _ in range(1, number):
                idx = int(dist.argmax())
                selected.append(idx)
                d = ((pts - pts[idx]) ** 2).sum(axis=1)
                dist = np.minimum(dist, d)
            fps_idx_list.append(selected)
        fps_idx = torch.tensor(np.array(fps_idx_list), dtype=torch.int64, device=data_c.device)

    assert fps_idx.is_cuda, "fps_idx must be on CUDA"
    assert fps_idx.dtype in (torch.int32, torch.int64), f"fps_idx dtype={fps_idx.dtype}"
    max_idx = int(fps_idx.max().item())
    min_idx = int(fps_idx.min().item())
    assert 0 <= min_idx, f"fps_idx min_idx={min_idx} < 0"
    assert max_idx < N, f"fps_idx max_idx={max_idx} >= N={N} (out of range)"

    # gather_operation expects contiguous input tensors (C++ extension requirement)
    fps_data = pointnet2_utils.gather_operation(
        data_c.transpose(1, 2).contiguous(),  # [B,N,C] -> make contiguous
        fps_idx
    ).transpose(1, 2).contiguous()  # [B,number,C] -> [B,C,number]
...
class PointTransformer(nn.Module):
    ...
    def load_model_from_ckpt(self, bert_ckpt_path):
        if bert_ckpt_path is not None:
            ckpt = _load_ckpt(bert_ckpt_path)

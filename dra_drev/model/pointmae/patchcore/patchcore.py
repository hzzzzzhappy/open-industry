...
import model.pointmae.patchcore as patchcore
import model.pointmae.patchcore.backbones
import model.pointmae.patchcore.common
import model.pointmae.patchcore.sampler
...
from model.pointmae.M3DM.cpu_knn import fill_missing_values
from model.pointmae.feature_extractors.ransac_position import get_registration_np,get_registration_refine_np
from model.pointmae.utils.utils import get_args_point_mae
from model.pointmae.M3DM.models import Model1
...
class PatchCore(torch.nn.Module):
    ...
    def load(
        ...
        basic_template=None,
        xyz_backbone_name='Point_MAE',#point_bert
        **kwargs,
    ):
        self.xyz_backbone_name = xyz_backbone_name
    ...
    def update_basic_template(self,basic_template):
        # assert 
        self.basic_template = basic_template

    def set_deep_feature_extractor(self):
        # args = get_args_point_mae()
        self.deep_feature_extractor = Model1(device=self.device, 
                        rgb_backbone_name='vit_base_patch8_224_dino', 
                        # xyz_backbone_name='Point_MAE', 
                        xyz_backbone_name=self.xyz_backbone_name,
                        group_size = 128, 
                        num_group = 16384)
    ...
    def _embed_pointmae(self, point_cloud, detach=True):
        """
        Args:
            point_cloud: Tensor (1, N, 3) or (N, 3); already normalized/registered upstream.
        Returns:
            pmae_features: float32 numpy (N_PTS, C)
            center_idx: indices on the original device
        """
        orig_device = point_cloud.device

        if detach:
            point_cloud = point_cloud.detach()

        pc_np = point_cloud.squeeze(0).to("cpu").numpy().astype(np.float32)
        if pc_np.ndim != 2 or pc_np.shape[1] != 3:
            raise ValueError(f"[PointMAE] Invalid point_cloud shape: {pc_np.shape}, expect (N,3)")

        N_raw = pc_np.shape[0]
        
        MIN_POINTS = 256
        if N_raw < MIN_POINTS:
            print(f"[WARN] point_cloud too few points ({N_raw}); resampling with replacement to target N")

        pc_np = pc_np.astype(np.float32)

        pointcloud_data = (
            torch.from_numpy(pc_np)    # (N_PTS, 3)
            .permute(1, 0)             # (3, N_PTS)
            .unsqueeze(0)              # (1, 3, N_PTS)
            .to(orig_device, dtype=torch.float32)
        )

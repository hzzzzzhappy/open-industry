...
def farthest_point_sampling(points, num_samples):
    ...
    # Record distance from all points to nearest point in sample set
    distances = np.full(N, np.inf)

    for i in range(1, num_samples):
        # Update minimum distance from all points to current sample set
        dist = np.linalg.norm(points - sampled_points[i - 1], axis=1)
        distances = np.minimum(distances, dist)

        # Select farthest point as next sample point
...
def compute_per_point_cvfh(points, normals, bins=45):
    ...
    for i in range(points.shape[0]):
        ...
        # Compute angle distribution for current point and its neighborhood
